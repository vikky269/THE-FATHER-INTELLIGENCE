...
/**
 * US data window - CPI / NFP releases
 * markets - flash - 30 12 * * 1-5 (UTC)
 *
...
export default async function handler() {
  ...
  if (!site || !token) {
    console.error("[markets-flash] SITE_URL or GENERATE_API_TOKEN missing - skipping run.");
    return;
  }

  const res = await fetch(`${site}/api/generate?desk=markets&slot=flash`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
  });

  console.log(`[markets-flash] ${res.status} ${(await res.text()).slice(0, 300)}`);
}

export const config: Config = { schedule: "30 12 * * 1-5" };
