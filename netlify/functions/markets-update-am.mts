...
/**
 * Mid-morning live update
 * markets - update - 0 9 * * 1-5 (UTC)
 *
...
export default async function handler() {
  ...
  if (!site || !token) {
    console.error("[markets-update-am] SITE_URL or GENERATE_API_TOKEN missing - skipping run.");
    return;
  }
  ...
  console.log(`[markets-update-am] ${res.status} ${(await res.text()).slice(0, 300)}`);
}

export const config: Config = { schedule: "0 9 * * 1-5" };
